...
function HomePage() {
  ...
  // Your function to navigate to the OrderDetail page with order and docRef
  ...
  // const handleDetele = async (order) => {
  //   const ordersRef = collection(db, "orderList");
  //   const q = query(ordersRef);
  //   const querySnapshot = await getDocs(q);
  //   if (!querySnapshot.empty) {
  //     const docRef = doc(db, "orderList", querySnapshot.docs[0].id);
  //     await deleteDoc(docRef);
  //   }
  //   await deleteDoc(doc(db, "orderList", order.id));
  // };
  ...
  return (
    <div className="container">
      ...
      <div className="refresh" onClick={getOrders}>
